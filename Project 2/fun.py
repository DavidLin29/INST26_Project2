...
class MainWindow(tk.Tk):
    ...
    def open_notebook_window(self):
        notebook_window = NotebookWindow(self.notes, self)
    ...
    def display_notes(self):
        ...
        for i, note in enumerate(self.notes):
            ...
            edit_button = tk
            edit_button = tk.Button(note_frame, text="Edit", command=lambda idx=i: self.edit_note(idx))
            edit_button.grid(row=0, column=1, rowspan=3, padx=5, pady=5)

    def edit_note(self, idx):
        edit_window = EditNoteWindow(self, idx)
        edit_window.mainloop()

class NotebookWindow(tk.Toplevel):
    def __init__(self, notes, main_window):
        super().__init__()
...
class EditNoteWindow(tk.Toplevel):
    def __init__(self, main_window, idx):
        ...
        note = self.main_window.notes[idx]
        ...
        self.text_label = tk.Label(self, text='Note Text:')
    ...
    def save_note(self):
        title = self.title_entry.get()
        text = self.text_entry.get('1.0', 'end').strip()
        created = self.main_window.notes[self.idx].created
        self.main_window.notes[self.idx] = Note(title, text, created)
        self.main_window.display_notes()
        self.destroy()
